...
<%
    // Auth + role check — COPY-PASTED from daily-sales.jsp and sales-by-book.jsp!
...
<head>
    ...
    <style>
        ...
        /* Additional: rank badge for top books */
        .rank-badge { display: inline-block; width: 24px; height: 24px; line-height: 24px; text-align: center;
                      border-radius: 50%; background: #ffc107; color: #333; font-weight: bold; font-size: 11px; }
...
<body>
<div class="container">
    <h2>Top Books Report</h2>
    <% if (errMsg != null) { %><div class="err"><%= errMsg %></div><% } %>

    
    <div class="report-header">
        <form action="/legacy-app/reports/topbooks.do" method="get">
            <input type="hidden" name="method" value="topBooks">
            <label>From:</label>
    ...
    <div class="export-bar">
        <a href="/legacy-app/export/csv.do?method=exportCsv&reportType=topbooks&startDate=<%= startDate != null ? startDate : "" %>&endDate=<%= endDate != null ? endDate : "" %>"
    ...
    <table class="report-table">
        ...
        <%
            // COPY-PASTED loop from daily-sales.jsp!
            for (int i = 0; i < reportData.size(); i++) {
                String[] row = (String[]) reportData.get(i);
                // row[0]=isbn, row[1]=title, row[2]=category, row[3]=qtySold, row[4]=revenue
                double revenue = 0.0;
        ...
        %>
        <tr class="<%= i % 2 == 0 ? "" : "even" %>">
            <td class="number"><span class="rank-badge <%= rankClass %>"><%= i + 1 %></span></td>
    ...
    <p style="font-size:10px; color:#999;">Top <%= reportData.size() %> books | Period: <%= startDate %> to <%= endDate %></p>
    <% } else { %>
